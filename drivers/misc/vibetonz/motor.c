...
struct regulator *motor_regulator;

void motor_control(bool onOff)
{
	if (onOff)
		regulator_enable(motor_regulator);
	else {
		if (regulator_is_enabled(motor_regulator))
			regulator_force_disable(motor_regulator);
	}
}
...
static void enable_vibetonz_from_user(struct timed_output_dev *dev,int value)
{
	...
	if (value > 0) 
	{
		schedule_delayed_work(&delayed_write_work,  msecs_to_jiffies(value));
...
{
	motor_regulator = regulator_get(NULL, "vmotor");
	if (IS_ERR(motor_regulator)) {
		printk(KERN_ERR "vmotor_regulator_get fail\n");
		return;
	}

	INIT_DELAYED_WORK(&delayed_write_work, vibetonz_timer_func);
