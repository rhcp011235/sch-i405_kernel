...
{
	/* for TA setting */
	noise_suppression_config_e_TA.ctrl = 1;//0
	noise_suppression_config_e_TA.cfg = 132;// 1
	noise_suppression_config_e_TA.calcfg = 80;// 2
	...
	noise_suppression_config_e_TA.mffreq[0] = 0;// 8
	noise_suppression_config_e_TA.mffreq[1] = 0;// 9
	...
	noise_suppression_config_e_TA.mferrorthr = 38;//25
...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	if (info_block->info_id.family_id == 0x80) {
		...
		noise_suppression_config_e.ctrl = 1;//0
		...
		noise_suppression_config_e.mffreq[0] = 0;// 8
		noise_suppression_config_e.mffreq[1] = 0;// 9
		...
		noise_suppression_config_e.mferrorthr = 38;//25
...
void qt_Touch_Suppression_Config_Init(void)
{
	if (info_block->info_id.family_id == 0x80) {
		...
		touch_suppression_config.ctrl = 0;
		touch_suppression_config.apprthr = 32;
		touch_suppression_config.maxapprarea = 120;
		touch_suppression_config.maxtcharea = 100;
		touch_suppression_config.supstrength = 0;
		touch_suppression_config.supextto = 0;
		touch_suppression_config.maxnumtchs = 0;
		touch_suppression_config.shapestrength = 0;
	...
	if ((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00)) {
		...
		* page byte will sit at 1 until we next send F3 command */
		...
		if ((tch_ch > 0) && (atch_ch == 0)) {  //jwlee change.
			...
			if (qt_timer_state == 1) {
				if (qt_time_diff > 400) { //jun change 11.5.3
...
void  get_message(void)
{
	...
	printk("msg id=%x %x %x %x %x\n",quantum_msg[0],quantum_msg[1], quantum_msg[2]&0xF,quantum_msg[3]&0xF,quantum_msg[4]);
